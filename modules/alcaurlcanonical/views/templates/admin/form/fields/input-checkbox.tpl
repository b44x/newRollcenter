            ...
            <div class="checkbox{if isset($input.expand) && strtolower($input.expand.default) == 'show'} hidden{/if}">
                {strip}
                    <label for="{$id_checkbox|escape:'html':'UTF-8'}">
                        ...
                        class="{$id_checkbox|escape:'html':'UTF-8'}"{if isset($value[$input.values.id])} 
                        value="{$value[$input.values.id]|escape:'html':'UTF-8'}"{/if}
                        ...
                        {if isset($input.required) && $input.required && isset($input.required_values) && in_array($value[$input.values.id], $input.required_values)}<label class="required_value"></label>{/if} {$value[$input.values.name]|escape:'html':'UTF-8'}
                    </label>
                {/strip}
            </div>
        {/foreach}
