...
abstract class Document extends Controls_Stack {
	...
	public static function get_editor_panel_config() {
		return [
			...
			'messages' => [
				...
				'publish_notification' => sprintf( Leo_Helper::__( 'Hurray! Your %s is live.', 'elementor' ), static::get_title() ),
			],
		];
	}
	...
	public static function get_title() {
		return Leo_Helper::__( 'Document', 'elementor' );
	}
	...
	public function get_wp_preview_url() {
		 ...
		 */
		$url = Leo_Helper::apply_filters( 'elementor/document/urls/wp_preview', $url, $this );
	...
	public function get_exit_to_dashboard_url() {
		 ...
		 * @param Document $this The document instance.
		 */
		$url = Leo_Helper::apply_filters( 'elementor/document/urls/exit_to_dashboard', $url, $this );
	...
	public function is_autosave() {
		return wp_is_post_autosave( Leo_Helper::$id_post );
	}
	...
	public function filter_admin_row_actions( $actions ) {
		if ( $this->is_built_with_elementor() && $this->is_editable_by_current_user() ) {
			$actions['edit_with_elementor'] = sprintf(
				'<a href="%1$s">%2$s</a>',
				$this->get_edit_url(),
				Leo_Helper::__( 'Edit with Elementor', 'elementor' )
			);
		}
	...
	protected function _register_controls() {
		$this->start_controls_section(
			'document_settings',
			[
				'label' => Leo_Helper::__( 'General Settings', 'elementor' ),
		...
		$this->add_control(
			...
			[
				...
				'default' => Leo_Helper::$post_title ? Leo_Helper::$post_title : '',
				'label_block' => true,
				'separator' => 'none',
			]
		);
		...
		$this->add_control(
			'post_status',
			[
				'label'   => Leo_Helper::__( 'Status', 'elementor' ),
				'type'    => Controls_Manager::SELECT,
				'default' => Leo_Helper::$post_status ? Leo_Helper::$post_status : 'publish',
				'options' => [ 'publish' => Leo_Helper::__( 'Published' ), 'private' => Leo_Helper::__( 'Hidden' ) ],
			]
		);
		 ...
		 * @param Document $this The document instance.
		 */
		Leo_Helper::do_action( 'elementor/documents/register_controls', $this );
	}
	...
	public function save( $data ) {
		 ...
		 * @param $data.
		 */
		Leo_Helper::do_action( 'elementor/document/before_save', $this, $data );
		...
		// Remove Post CSS
		$post_css = Post_CSS::create( Leo_Helper::$id_post );
		 ...
		 * @param $data.
		 */
		Leo_Helper::do_action( 'elementor/document/after_save', $this, $data );
	...
	public function is_built_with_elementor() {
		return ! ! Leo_Helper::get_post_meta( $this->post->ID, '_elementor_edit_mode', true );
	}
	...
	public function get_edit_url() {
		 ...
		 * @param Document $this The document instance.
		 */
		$url = Leo_Helper::apply_filters( 'elementor/document/urls/edit', $url, $this );
	...
	public function get_preview_url() {
		...
		if ( empty( $url ) ) {
			...
			Leo_Helper::add_filter( 'pre_option_permalink_structure', '__return_empty_string' );

			$url = Leo_Helper::set_url_scheme( add_query_arg( [
				'elementor-preview' => $this->get_main_id(),
				'ver' => time(),
			], $this->get_permalink() ) );

			Leo_Helper::remove_filter( 'pre_option_permalink_structure', '__return_empty_string' );
			 ...
			 * @param Document $this The document instance.
			 */
			$url = Leo_Helper::apply_filters( 'elementor/document/urls/preview', $url, $this );
		}
	...
	public function get_json_meta( $key ) {
		$meta = Leo_Helper::get_post_meta( $this->post->ID, $key, true );
	...
	public function get_elements_data( $status = DB::STATUS_PUBLISH ) {
		$obj =  new \LeoElementsContentsModel( Leo_Helper::$id_post, Leo_Helper::$id_lang );
		...
		if( Leo_Helper::$id_post != Leo_Helper::$id_editor && !$obj->active ){
			return [];
		}elseif( \Tools::getValue( 'wp_preview' ) == Leo_Helper::$id_post ){
	...
	public function print_elements_with_wrapper( $elements_data = null ) {
		...
		$attributes = [
			...
			'class' => 'elementor elementor-' . ( Leo_Helper::$is_template ? 'temp-' : '' ) . $id_post,
		];
	...
	public function get_panel_page_settings() {
		return [
			...
			'title' => sprintf( Leo_Helper::__( '%s Settings', 'elementor' ), static::get_title() ),
		];
	}
	...
	public function get_content( $with_css = false ) {
		return Plugin::$instance->frontend->get_builder_content( Leo_Helper::$id_post, $with_css );
	}
	...
	public function delete() {
		...
		return $deleted && ! Leo_Helper::is_wp_error( $deleted );
	}
	...
	public function save_version() {
		if ( ! defined( 'IS_LEOELEMENTS_UPGRADE' ) ) {
			// Save per revision.
			$this->update_meta( '_elementor_version', LEOELEMENTS_VERSION );
			 ...
			 *
			 */
			Leo_Helper::do_action( 'elementor/document/save_version', $this );
		}
	}
	...
	public function get_main_meta( $key ) {
		return Leo_Helper::get_post_meta( Leo_Helper::$id_post, $key, true );
	}
	...
	public function update_main_meta( $key, $value ) {
		return Leo_Helper::update_post_meta( Leo_Helper::$id_post, $key, $value );
	}
	...
	public function delete_main_meta( $key, $value = '' ) {
		return Leo_Helper::delete_post_meta( Leo_Helper::$id_post, $key, $value );
	}
	...
	public function get_meta( $key ) {
		return Leo_Helper::get_post_meta( $this->post->ID, $key, true );
	}
	...
	public function get_last_edited() {
		...
		$date = date_i18n( Leo_Helper::_x( 'M j, H:i', 'revision date format', 'elementor' ), strtotime( $post->post_modified ) );
		$display_name = get_the_author_meta( 'display_name', $post->post_author );

		if ( $autosave_post || 'revision' === $post->post_type ) {
			/* translators: 1: Saving date, 2: Author display name */
			$last_edited = sprintf( Leo_Helper::__( 'Draft saved on %1$s by %2$s', 'elementor' ), '<time>' . $date . '</time>', $display_name );
		} else {
			/* translators: 1: Editing date, 2: Author display name */
			$last_edited = sprintf( Leo_Helper::__( 'Last edited on %1$s by %2$s', 'elementor' ), '<time>' . $date . '</time>', $display_name );
		}
	...
	protected function save_settings( $settings ) {
		...
		$page_settings_manager->save_settings( $settings, Leo_Helper::$id_post );
	}
	...
	private function save_usage( $elements ) {
		...
		$global_usage = Leo_Helper::get_option( self::ELEMENTS_USAGE_OPTION_NAME, [] );
		...
		Leo_Helper::update_option( self::ELEMENTS_USAGE_OPTION_NAME, $global_usage );
	}
}