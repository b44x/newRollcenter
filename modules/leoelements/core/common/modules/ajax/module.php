...
namespace LeoElements\Core\Common\Modules\Ajax;
...
class Module extends BaseModule {
	...
	public function __construct() {
		Leo_Helper::add_action( 'wp_ajax_elementor_ajax', [ $this, 'handle_ajax_request' ] );
	}
	...
	public function handle_ajax_request() {
		 ...
		 */
		Leo_Helper::do_action( 'elementor/ajax/register_actions', $this );
		...
		foreach ( $this->requests as $id => $action_data ) {
			...
			if ( ! isset( $this->ajax_actions[ $action_data['action'] ] ) ) {
				$this->add_response_data( false, Leo_Helper::__( 'Action not found.', 'elementor' ), Exceptions::BAD_REQUEST );
	...
	protected function get_init_settings() {
		return [
			'url'   => Leo_Helper::get_ajax_editor(),
			'nonce' => '',
		];
	}
