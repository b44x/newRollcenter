...
class Documents_Manager {
	...
	public function __construct() {
		Leo_Helper::add_action( 'elementor/documents/register', [ $this, 'register_default_types' ], 0 );
		Leo_Helper::add_action( 'elementor/ajax/register_actions', [ $this, 'register_ajax_actions' ] );
		Leo_Helper::add_filter( 'post_row_actions', [ $this, 'filter_post_row_actions' ], 11, 2 );
		Leo_Helper::add_filter( 'page_row_actions', [ $this, 'filter_post_row_actions' ], 11, 2 );
		Leo_Helper::add_filter( 'user_has_cap', [ $this, 'remove_user_edit_cap' ], 10, 3 );
		Leo_Helper::add_filter( 'elementor/editor/localize_settings', [ $this, 'localize_settings' ] );
	}
	...
	public function create( $type, $post_data = [], $meta_data = [] ) {
		...
		if ( empty( $post_data['post_title'] ) ) {
			$post_data['post_title'] = Leo_Helper::__( 'Elementor', 'elementor' );
			if ( 'post' !== $type ) {
				$post_data['post_title'] = sprintf(
					/* translators: %s: Document title */
					Leo_Helper::__( 'Elementor %s', 'elementor' ),
	...
	public function ajax_save( $request ) {
		...
		if( $key_related && $post_type != 'hook' ){
			$related = Leo_Helper::getRelatedByKey();
			$obj = new \LeoElementsRelated( (int)$related['id_leoelements_related'], $id_lang, $id_shop );
			$obj->id_post = $id_post;
		...
		if( $request['status'] != DB::STATUS_AUTOSAVE ){
			$revision = new \LeoElementsRevisions();			
		...
		$post->id_employee = Leo_Helper::get_current_user_id();
		$post->content = json_encode( $request['elements'] );
		
		if( $request['status'] == DB::STATUS_AUTOSAVE ){
			$autosave = new \LeoElementsContentsModel( $id_post, $id_lang );
		...
		$return_data = [
			'config' => [
				'document' => [
					...
					'urls' => [
						'wp_preview' => Leo_Helper::get_permalink( [ 'wp_preview' => $id_post ] ),
					],
				],
			],
		];
		 ...
		 */
		$return_data = Leo_Helper::apply_filters( 'elementor/documents/ajax_save/return_data', $return_data, $document );
	...
	private function register_types() {
		Leo_Helper::do_action( 'elementor/documents/register', $this );
	}
}