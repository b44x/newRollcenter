...
namespace LeoElements\Core\Files\Assets\Svg;
...
class Svg_Handler {
	...
	public static function is_svg_uploads_enabled() {
		return ! ! Leo_Helper::get_option( 'elementor_allow_svg', false );
	}
	...
	protected function get_meta() {
		return Leo_Helper::get_post_meta( $this->attachment_id, self::META_KEY, true );
	}
	...
	protected function update_meta( $meta ) {
		Leo_Helper::update_post_meta( $this->attachment_id, self::META_KEY, $meta );
	}
	...
	protected function delete_meta() {
		Leo_Helper::delete_post_meta( $this->attachment_id, self::META_KEY );
	}
	...
	public function delete_meta_cache() {
		Leo_Helper::delete_post_meta_by_key( self::META_KEY );
	}
	...
	public function wp_handle_upload_prefilter( $file ) {
		...
		if ( 'svg' !== $ext ) {
			$file['error'] = sprintf( Leo_Helper::__( 'The uploaded %s file is not supported. Please upload a valid svg file', 'elementor' ), $ext );
			return $file;
		}

		if ( ! self::is_enabled() ) {
			$file['error'] = Leo_Helper::__( 'SVG file is not allowed for security reasons', 'elementor' );
			return $file;
		}

		if ( self::svg_sanitizer_can_run() && ! $this->sanitize_svg( $file['tmp_name'] ) ) {
			$file['error'] = Leo_Helper::__( 'Invalid SVG Format, file not uploaded for security reasons', 'elementor' );
		}
	...
	private function is_elementor_media_upload() {
            $post = $GLOBALS['_POST'];
		return isset( $post['uploadTypeCaller'] ) && 'elementor-editor-upload' === $post['uploadTypeCaller']; // phpcs:ignore
	}
	...
	private function get_allowed_attributes() {
		...
		return Leo_Helper::apply_filters( 'elementor/files/svg/allowed_attributes', $allowed_attributes );
	}
	...
	private function get_allowed_elements() {
		...
		return Leo_Helper::apply_filters( 'elementor/files/svg/allowed_elements', $allowed_elements );
	}
...
//		$libxml_use_internal_errors = libxml_use_internal_errors( true );
		$libxml_use_internal_errors = call_user_func('libxml_use_internal_errors', true);
...
//		libxml_use_internal_errors( $libxml_use_internal_errors );
                ...
                call_user_func('libxml_use_internal_errors', $libxml_use_internal_errors);
	...
	public static function is_enabled() {
		...
		if ( null === $enabled ) {
			...
			$enabled = Leo_Helper::apply_filters( 'elementor/files/svg/enabled', $enabled );
		}
		return $enabled;
	}
	...
	public function __construct() {
		Leo_Helper::add_filter( 'upload_mimes', [ $this, 'upload_mimes' ] );
		Leo_Helper::add_filter( 'wp_handle_upload_prefilter', [ $this, 'wp_handle_upload_prefilter' ] );
		Leo_Helper::add_filter( 'wp_check_filetype_and_ext', [ $this, 'wp_check_filetype_and_ext' ], 10, 4 );
		Leo_Helper::add_filter( 'wp_prepare_attachment_for_js', [ $this, 'wp_prepare_attachment_for_js' ], 10, 3 );
		Leo_Helper::add_action( 'elementor/core/files/clear_cache', [ $this, 'delete_meta_cache' ] );
	}
}
