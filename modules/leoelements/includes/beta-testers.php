...
class Beta_Testers {
	...
	private function get_beta_version() {
		...
		if ( false === $beta_version ) {
			...
			$response = Leo_Helper::wp_remote_get( 'https://plugins.svn.wordpress.org/elementor/trunk/readme.txt' );

			if ( ! Leo_Helper::is_wp_error( $response ) && ! empty( $response['body'] ) ) {
			...
			set_site_transient( $this->transient_key, $beta_version, 6 * LEO_HOUR_IN_SECONDS );
		}
	...
	public function check_version( $transient ) {
		...
		if ( 'false' !== $beta_version && version_compare( $beta_version, LEOELEMENTS_VERSION, '>' ) ) {
			...
			$transient->response[ LEOELEMENTS_PLUGIN_BASE ] = $response;
		}
	...
	public function __construct() {
		if ( 'yes' !== Leo_Helper::get_option( 'elementor_beta', 'no' ) ) {
			return;
		}

		$this->transient_key = md5( 'elementor_beta_testers_response_key' );

		Leo_Helper::add_filter( 'pre_set_site_transient_update_plugins', [ $this, 'check_version' ] );
	}
}