...
class Compatibility {
	...
	public static function register_actions() {
		Leo_Helper::add_action( 'init', [ __CLASS__, 'init' ] );

		self::polylang_compatibility();

		Leo_Helper::add_action( 'elementor/maintenance_mode/mode_changed', [ __CLASS__, 'clear_3rd_party_cache' ] );
	}
	...
	public static function clear_3rd_party_cache() {
		...
		if ( function_exists( 'wp_cache_clean_cache' ) ) {
                        $file_prefix = &$GLOBALS['file_prefix'];
	...
	public static function init() {
		...
		if ( defined( 'NGG_PLUGIN_VERSION' ) ) {
			Leo_Helper::add_filter( 'elementor/document/urls/edit', function( $edit_link ) {
		...
		if ( class_exists( '\Ninja_Forms' ) && class_exists( '\NF_Display_Render' ) ) {
			Leo_Helper::add_action( 'elementor/preview/enqueue_styles', function() {
				ob_start();
		...
		Leo_Helper::add_filter( 'wpseo_sitemaps_supported_post_types', [ __CLASS__, 'filter_library_post_type' ] );
		Leo_Helper::add_filter( 'wpseo_accessible_post_types', [ __CLASS__, 'filter_library_post_type' ] );
		Leo_Helper::add_filter( 'wpseo_sitemap_exclude_post_type', function( $retval, $post_type ) {
		...
		Leo_Helper::add_filter( 'autoptimize_filter_noptimize', function( $retval ) {
		...
		Leo_Helper::add_filter( 'woocommerce_product_tabs', function( $tabs ) {
			if ( ! isset( $tabs['description'] ) && Plugin::$instance->preview->is_preview_mode() ) {
				$post = get_post();
				if ( empty( $post->post_content ) ) {
					$tabs['description'] = [
						'title' => Leo_Helper::__( 'Description', 'elementor' ),
						'priority' => 10,
		...
		if ( class_exists( 'woocommerce' ) ) {
			Leo_Helper::add_action( 'elementor/editor/before_enqueue_scripts', function() {
			...
			Leo_Helper::add_filter( 'elementor/maintenance_mode/is_login_page', function( $value ) {
		...
		if ( class_exists( 'domain_map' ) ) {
			Leo_Helper::add_filter( 'elementor/document/urls/preview', function( $preview_url ) {
	...
	private static function polylang_compatibility() {
		...
		if ( isset( $_REQUEST['action'] ) && 0 === strpos( $_REQUEST['action'], 'elementor' ) ) {
			Leo_Helper::add_action( 'set_current_user', function() {
			...
			Leo_Helper::add_action( 'pll_pre_init', function( $polylang ) {
		...
		Leo_Helper::add_filter( 'pll_copy_post_metas', [ __CLASS__, 'save_polylang_meta' ], 10, 4 );
	}
