...
class Group_Control_Image_Size extends Group_Control_Base {
	...
	public static function get_attachment_image_html( $settings, $image_size_key = 'image', $image_key = null ) {
		...
		if ( ! empty( $image_src ) ) {
			...
			$html .= sprintf( '<img src="%s" title="%s" alt="%s"%s />', Leo_Helper::esc_attr( $image_src ), $title, $alt, $image_class_html );
		}
		 ...
		 *                               as the image key.
		 */
		return Leo_Helper::apply_filters( 'elementor/image_size/get_attachment_image_html', $html, $settings, $image_size_key, $image_key );
	}
	...
	public static function get_all_image_sizes() {
		...
		foreach ( $default_image_sizes as $size ) {
			$image_sizes[ $size ] = [
				'width' => (int) Leo_Helper::get_option( $size . '_size_w' ),
				'height' => (int) Leo_Helper::get_option( $size . '_size_h' ),
				'crop' => (bool) Leo_Helper::get_option( $size . '_crop' ),
			];
		}
		...
		return Leo_Helper::apply_filters( 'image_size_names_choose', $image_sizes );
	}
	...
	public static function get_attachment_image_src( $attachment_id, $image_size_key, array $settings ) {
		...
		if ( 'custom' !== $size ) {
			...
			require_once( LEOELEMENTS_PATH . 'includes/libraries/bfi-thumb/bfi-thumb.php' );
	...
	protected function init_fields() {
		...
		$fields['size'] = [
			'label' => Leo_Helper::_x( 'Image Size', 'Image Size Control', 'elementor' ),
			'type' => Controls_Manager::SELECT,
			'label_block' => false,
		];

		$fields['custom_dimension'] = [
			'label' => Leo_Helper::_x( 'Image Dimension', 'Image Size Control', 'elementor' ),
			'type' => Controls_Manager::IMAGE_DIMENSIONS,
			'description' => Leo_Helper::__( 'You can crop the original image size to any custom size. You can also set a single value for height or width in order to keep the original size ratio.', 'elementor' ),
			'condition' => [
				'size' => 'custom',
			],
			'separator' => 'none',
	...
	private function get_image_sizes() {
		...
		$image_sizes['full'] = Leo_Helper::_x( 'Full', 'Image Size Control', 'elementor' );

		$image_sizes['custom'] = Leo_Helper::_x( 'Custom', 'Image Size Control', 'elementor' );
