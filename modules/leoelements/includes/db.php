...
class DB {
	...
	protected function _get_json_meta( $post_id, $key ) {
		$meta = Leo_Helper::get_post_meta( $post_id, $key, true );
	...
	public function set_is_elementor_page( $post_id, $is_elementor = true ) {
		if ( $is_elementor ) {
			...
			Leo_Helper::update_post_meta( $post_id, '_elementor_edit_mode', 'builder' );
		} else {
			Leo_Helper::delete_post_meta( $post_id, '_elementor_edit_mode' );
		}
	}
	...
	public function safe_copy_elementor_meta( $from_post_id, $to_post_id ) {
		...
		if ( ! did_action( 'elementor/db/before_save' ) ) {
			...
			if ( Leo_Helper::get_post_meta( $to_post_id, '_elementor_data', true ) ) {
				return;
			}
		}
	...
	public function copy_elementor_meta( $from_post_id, $to_post_id ) {
		$from_post_meta = Leo_Helper::get_post_meta( $from_post_id );
		$core_meta = [
			'_wp_page_template',
			'_thumbnail_id',
		];
	...
	public function is_built_with_elementor( $post_id ) {
		return ! ! Leo_Helper::get_post_meta( $post_id, '_elementor_edit_mode', true );
	}
	...
	public function switch_to_post( $post_id ) {
		...
		if ( Leo_Helper::get_the_ID() === $post_id ) {
		...
		$this->switched_post_data[] = [
			'switched_id' => $post_id,
			'original_id' => Leo_Helper::get_the_ID(), // Note, it can be false if the global isn't set
		];
