 ...
 */
Leo_Helper::add_filter( 'wp_image_editors', 'bfi_wp_image_editor' );
	...
	class BFI_Thumb_1_3 {
		...
		public static function thumb( $url, $params = array(), $single = true ) {
			...
			if ( @file_exists( $destfilename ) && getimagesize( $destfilename ) ) {
				...
				if ( Leo_Helper::is_wp_error( $editor ) ) {
					return false;
				}
				...
				if ( $crop_only === false ) {
					if ( ( isset( $width ) && $width ) || ( isset( $height ) && $height ) ) {
						if ( Leo_Helper::is_wp_error( $editor->resize( isset( $width ) ? $width : null, isset( $height ) ? $height : null, isset( $crop ) ? $crop : false ) ) ) {
							return false;
						}
					}
				} else {
					if ( Leo_Helper::is_wp_error( $editor->crop( $src_x, $src_y, $src_w, $src_h, $dst_w, $dst_h ) ) ) {
						return false;
					}
				}
				...
				if ( isset( $negate ) ) {
					if ( $negate ) {
						if ( Leo_Helper::is_wp_error( $editor->negate() ) ) {
							return false;
						}
					}
				}
				...
				if ( isset( $opacity ) ) {
					if ( Leo_Helper::is_wp_error( $editor->opacity( $opacity ) ) ) {
						return false;
					}
				}
				...
				if ( isset( $grayscale ) ) {
					if ( $grayscale ) {
						if ( Leo_Helper::is_wp_error( $editor->grayscale() ) ) {
							return false;
						}
					}
				}
				...
				if ( isset( $color ) ) {
					if ( Leo_Helper::is_wp_error( $editor->colorize( $color ) ) ) {
						return false;
					}
				}
...
if ( ! Leo_Helper::is_admin() ) {
	Leo_Helper::add_filter( 'image_resize_dimensions', 'bfi_image_resize_dimensions', 10, 5 );
}
...
Leo_Helper::add_filter( 'image_downsize', 'bfi_image_downsize', 1, 3 );
