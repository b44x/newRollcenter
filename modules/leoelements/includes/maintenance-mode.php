...
class Maintenance_Mode {
	...
	public static function get( $option, $default = false ) {
		return Leo_Helper::get_option( self::OPTION_PREFIX . $option, $default );
	}
	...
	public static function set( $option, $value ) {
		return Leo_Helper::update_option( self::OPTION_PREFIX . $option, $value );
	}
	...
	public function template_redirect() {
		...
		$is_login_page = Leo_Helper::apply_filters( 'elementor/maintenance_mode/is_login_page', false );
	...
	public function register_settings_fields( Tools $tools ) {
		...
		$tools->add_tab(
			'maintenance_mode', [
				'label' => Leo_Helper::__( 'Maintenance Mode', 'elementor' ),
				'sections' => [
					'maintenance_mode' => [
						'callback' => function() {
							echo '<h2>' . Leo_Helper::esc_html__( 'Maintenance Mode', 'elementor' ) . '</h2>';
							echo '<div>' . Leo_Helper::__( 'Set your entire website as MAINTENANCE MODE, meaning the site is offline temporarily for maintenance, or set it as COMING SOON mode, meaning the site is offline until it is ready to be launched.', 'elementor' ) . '</div>';
						},
						'fields' => [
							'maintenance_mode_mode' => [
								'label' => Leo_Helper::__( 'Choose Mode', 'elementor' ),
								'field_args' => [
									'type' => 'select',
									'options' => [
										'' => Leo_Helper::__( 'Disabled', 'elementor' ),
										self::MODE_COMING_SOON => Leo_Helper::__( 'Coming Soon', 'elementor' ),
										self::MODE_MAINTENANCE => Leo_Helper::__( 'Maintenance', 'elementor' ),
									],
									'desc' => '<div class="elementor-maintenance-mode-description" data-value="" style="display: none">' .
											  Leo_Helper::__( 'Choose between Coming Soon mode (returning HTTP 200 code) or Maintenance Mode (returning HTTP 503 code).', 'elementor' ) .
											  '</div>' .
											  '<div class="elementor-maintenance-mode-description" data-value="maintenance" style="display: none">' .
											  Leo_Helper::__( 'Maintenance Mode returns HTTP 503 code, so search engines know to come back a short time later. It is not recommended to use this mode for more than a couple of days.', 'elementor' ) .
											  '</div>' .
											  '<div class="elementor-maintenance-mode-description" data-value="coming_soon" style="display: none">' .
											  Leo_Helper::__( 'Coming Soon returns HTTP 200 code, meaning the site is ready to be indexed.', 'elementor' ) .
											  '</div>',
								],
							],
							'maintenance_mode_exclude_mode' => [
								'label' => Leo_Helper::__( 'Who Can Access', 'elementor' ),
								'field_args' => [
									'class' => 'elementor-default-hide',
									'type' => 'select',
									'std' => 'logged_in',
									'options' => [
										'logged_in' => Leo_Helper::__( 'Logged In', 'elementor' ),
										'custom' => Leo_Helper::__( 'Custom', 'elementor' ),
									],
								],
							],
							'maintenance_mode_exclude_roles' => [
								'label' => Leo_Helper::__( 'Roles', 'elementor' ),
								'field_args' => [
									'class' => 'elementor-default-hide',
							...
							'maintenance_mode_template_id' => [
								'label' => Leo_Helper::__( 'Choose Template', 'elementor' ),
								'field_args' => [
									'class' => 'elementor-default-hide',
									'type' => 'select',
									'show_select' => true,
	...
	public function add_menu_in_admin_bar( \WP_Admin_Bar $wp_admin_bar ) {
		$wp_admin_bar->add_node( [
			...
			'title' => Leo_Helper::__( 'Maintenance Mode ON', 'elementor' ),
		...
		$wp_admin_bar->add_node( [
			...
			'title' => Leo_Helper::__( 'Edit Template', 'elementor' ),
	...
	public function on_update_mode( $old_value, $value ) {
		if ( $old_value !== $value ) {
			Leo_Helper::do_action( 'elementor/maintenance_mode/mode_changed', $old_value, $value );
		}
	}
	...
	public function __construct() {
		Leo_Helper::add_action( 'update_option_elementor_maintenance_mode_mode', [ $this, 'on_update_mode' ], 10, 2 );
		...
		Leo_Helper::add_action( 'admin_bar_menu', [ $this, 'add_menu_in_admin_bar' ], 300 );
		Leo_Helper::add_action( 'admin_head', [ $this, 'print_style' ] );
		Leo_Helper::add_action( 'wp_head', [ $this, 'print_style' ] );

		// Priority = 11 that is *after* WP default filter `redirect_canonical` in order to avoid redirection loop.
		Leo_Helper::add_action( 'template_redirect', [ $this, 'template_redirect' ], 11 );
	}
	...
	private function print_template_description() {
		...
		?>
		<a target="_blank" class="elementor-edit-template" style="display: none" href="<?php echo $edit_url; ?>"><?php echo Leo_Helper::__( 'Edit Template', 'elementor' ); ?></a>
		<div class="elementor-maintenance-mode-error"><?php echo Leo_Helper::__( 'To enable maintenance mode you have to set a template for the maintenance mode page.', 'elementor' ); ?></div>
		<div class="elementor-maintenance-mode-error"><?php echo sprintf( Leo_Helper::__( 'Select one or go ahead and <a target="_blank" href="%s">create one</a> now.', 'elementor' ), Leo_Helper::admin_url( 'post-new.php?post_type=' . Source_Local::CPT ) ); ?></div>
		<?php
	}
}