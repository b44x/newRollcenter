...
class Preview {
	...
	public function init() {
		if ( Leo_Helper::is_admin() || ! $this->is_preview_mode() ) {
			return;
		}

		if ( Tools::getIsset('preview-debug') ) {
		...
		$this->post_id = Leo_Helper::$id_post;
		...
		Leo_Helper::add_filter( 'show_admin_bar', '__return_false' );

		Leo_Helper::add_action( 'wp_enqueue_scripts', function() {
			$this->enqueue_styles();
			$this->enqueue_scripts();
		} );

		Leo_Helper::add_filter( 'the_content', [ $this, 'builder_wrapper' ], 999999 );

		Leo_Helper::add_action( 'wp_footer', [ $this, 'wp_footer' ] );

		// Avoid Cloudflare's Rocket Loader lazy load the editor iframe
		Leo_Helper::add_filter( 'script_loader_tag', [ $this, 'rocket_loader_filter' ], 10, 3 );
		 ...
		 */
		Leo_Helper::do_action( 'elementor/preview/init', $this );
	}
	...
	public function is_preview_mode( $post_id = 0 ) {
		...
		if ( ! Tools::getIsset('elementor-preview') || $post_id !== (int) Tools::getValue('elementor-preview') ) {
			return false;
		}
	...
	public function builder_wrapper( $content ) {
		if ( Leo_Helper::$id_post === $this->post_id ) {
	...
	private function enqueue_styles() {
		...
		Leo_Helper::wp_register_style(
			'editor-preview',
			LEOELEMENTS_ASSETS_URL . 'css/editor-preview' . $direction_suffix . $suffix . '.css',
			[
				'elementor-select2',
			],
			LEOELEMENTS_VERSION
		...
		Leo_Helper::wp_enqueue_style( 'editor-preview' );
		 ...
		 * @since 1.0.0
		 */
		Leo_Helper::do_action( 'elementor/preview/enqueue_styles' );
	}
	...
	private function enqueue_scripts() {
		...
		Leo_Helper::wp_enqueue_script(
			...
			LEOELEMENTS_ASSETS_URL . 'lib/inline-editor/js/inline-editor' . $suffix . '.js',
			[],
			LEOELEMENTS_VERSION,
			true
		);
		 ...
		 * @since 1.5.4
		 */
		Leo_Helper::do_action( 'elementor/preview/enqueue_scripts' );
	}
	...
	public function __construct() {
		Leo_Helper::add_action( 'template_redirect', [ $this, 'init' ], 0 );
	}
}