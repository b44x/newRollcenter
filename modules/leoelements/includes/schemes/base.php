...
abstract class Scheme_Base implements Scheme_Interface {
	...
	public function get_scheme_value() {
		$scheme_value = Leo_Helper::get_option( 'elementor_scheme_' . static::get_type() );

		if ( ! $scheme_value ) {
			$scheme_value = $this->get_default_scheme();

			Leo_Helper::update_option( 'elementor_scheme_' . static::get_type(), $scheme_value );
		}
	...
	public function save_scheme( array $posted ) {
		...
		Leo_Helper::update_option( 'elementor_scheme_' . static::get_type(), array_replace( $scheme_value, array_intersect_key( $posted, $scheme_value ) ) );

		Leo_Helper::update_option( self::LAST_UPDATED_META, time() );
			
		if( static::get_type() === 'typography' ){
			Leo_Helper::delete_option( '_elementor_global_css' );
		}
	}
			...
			<div class="elementor-panel-scheme-buttons">
				<div class="elementor-panel-scheme-button-wrapper elementor-panel-scheme-reset">
					<button class="elementor-button">
						...
						<?php echo Leo_Helper::__( 'Reset', 'elementor' ); ?>
					</button>
				</div>
				...
				<div class="elementor-panel-scheme-button-wrapper elementor-panel-scheme-save">
					<button class="elementor-button elementor-button-success" disabled><?php echo Leo_Helper::__( 'Apply', 'elementor' ); ?></button>
				</div>
			</div>
