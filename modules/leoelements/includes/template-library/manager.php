...
use LeoElements\TemplateLibrary\Classes\Import_Images;
...
class Manager {
	...
	public function add_actions() {
		...
		Leo_Helper::add_action( 'wp_ajax_elementor_library_direct_actions', [ $this, 'handle_direct_actions' ] );
                $post = $GLOBALS['_POST'];
		// TODO: bc since 2.3.0
		Leo_Helper::add_action( 'wp_ajax_elementor_update_templates', function() {
			if ( ! isset( $post['templates'] ) ) {
				return;
			}

			foreach ( $post['templates'] as & $template ) {
			...
			wp_send_json_success( $this->handle_ajax_request( 'update_templates', $post ) );
		} );
	}
	...
	public function save_template( array $args ) {
		...
		if ( Leo_Helper::is_wp_error( $validate_args ) ) {
	...
	public function update_template( array $template_data ) {
		...
		if ( Leo_Helper::is_wp_error( $update ) ) {
			return $update;
		}
	...
	public function update_templates( array $args ) {
		foreach ( $args['templates'] as $template_data ) {
			...
			if ( Leo_Helper::is_wp_error( $result ) ) {
				return $result;
			}
		}
	...
	public function get_template_data( array $args ) {
		...
		Leo_Helper::do_action( 'elementor/template-library/before_get_source_data', $args, $source );

		$data = $source->get_data( $args );

		Leo_Helper::do_action( 'elementor/template-library/after_get_source_data', $args, $source );
	...
	private function on_direct_import_template_success() {
		wp_safe_redirect( Leo_Helper::admin_url( Source_Local::ADMIN_MENU_SLUG ) );
	}
