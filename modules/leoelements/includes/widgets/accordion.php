...
class Widget_Accordion extends Widget_Base {
	...
	public function get_title() {
		return Leo_Helper::__( 'Accordion', 'elementor' );
	}
	...
	protected function _register_controls() {
		$this->start_controls_section(
			'section_title',
			[
				'label' => Leo_Helper::__( 'Accordion', 'elementor' ),
			]
		);
		...
		$repeater->add_control(
			...
			[
				...
				'default' => Leo_Helper::__( 'Accordion Title', 'elementor' ),
				'label_block' => true,
			]
		);
		...
		$repeater->add_control(
			...
			[
				...
				'default' => Leo_Helper::__( 'Accordion Content', 'elementor' ),
				'show_label' => false,
			]
		);
		...
		$this->add_control(
			'tabs',
			[
				'label' => Leo_Helper::__( 'Accordion Items', 'elementor' ),
				'type' => Controls_Manager::REPEATER,
				'fields' => $repeater->get_controls(),
				'default' => [
					[
						'tab_title' => Leo_Helper::__( 'Accordion #1', 'elementor' ),
					...
					[
						'tab_title' => Leo_Helper::__( 'Accordion #2', 'elementor' ),
						'tab_content' => Leo_Helper::__( 'Click edit button to change this text. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut elit tellus, luctus nec ullamcorper mattis, pulvinar dapibus leo.', 'elementor' ),
					],
				],
			...
			foreach ( $settings['tabs'] as $index => $item ) :
				...
				<div class="elementor-accordion-item">
					<<?php echo $settings['title_html_tag']; ?> <?php echo $this->get_render_attribute_string( $tab_title_setting_key ); ?>>
						<?php if ( $has_icon ) : ?>
							<span class="elementor-accordion-icon elementor-accordion-icon-<?php echo Leo_Helper::esc_attr( $settings['icon_align'] ); ?>" aria-hidden="true">
							<?php
							if ( $is_new || $migrated ) { ?>
							...
							<?php } else { ?>
								<i class="elementor-accordion-icon-closed <?php echo Leo_Helper::esc_attr( $settings['icon'] ); ?>"></i>
								<i class="elementor-accordion-icon-opened <?php echo Leo_Helper::esc_attr( $settings['icon_active'] ); ?>"></i>
							<?php } ?>
							</span>
						<?php endif; ?>
