...
class Widget_Menu_Anchor extends Widget_Base {
	...
	public function get_title() {
		return Leo_Helper::__( 'Menu Anchor', 'elementor' );
	}
	...
	protected function _register_controls() {
		$this->start_controls_section(
			'section_anchor',
			[
				'label' => Leo_Helper::__( 'Anchor', 'elementor' ),
			]
		);
		...
		$this->add_control(
			'anchor',
			[
				'label' => Leo_Helper::__( 'The ID of Menu Anchor.', 'elementor' ),
				'type' => Controls_Manager::TEXT,
				'placeholder' => Leo_Helper::__( 'For Example: About', 'elementor' ),
				'description' => Leo_Helper::__( 'This ID will be the CSS ID you will have to use in your own page, Without #.', 'elementor' ),
				'label_block' => true,
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'raw' => sprintf( Leo_Helper::__( 'Note: The ID link ONLY accepts these chars: %s', 'elementor' ), '`A-Z, a-z, 0-9, _ , -`' ),
	...
	protected function render() {
		...
		if ( ! empty( $anchor ) ) {
			$this->add_render_attribute( 'inner', 'id', Leo_Helper::sanitize_html_class( $anchor ) );
		}
