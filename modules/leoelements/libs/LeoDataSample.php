...
class Datasample
{
    ...
    public function __construct()
    {
        ...
        $this->_id_shop = $this->context->shop->id;
        ...
        $this->_theme_dir = leoECHelper::getThemeName().'/';
    ...
    {
        $theme_name = leoECHelper::getInstallationThemeName() . '/';
        ...
        if (!$moduleName) {
            $moduleList = Tools::getValue("leomodule");
        } else {
            $moduleList[] = $moduleName;
        }
        $_SERVER['processImport'] = 1;
        $error = 0;
...
//                    if ($moduleName == 'leoelements') {
//                        leoECHelper::leoCreateColumn('leoelements', 'id_leoelements_shortcode', 'int(11) NOT NULL');
//                    }
                }
                ...
                if ($module == "leoformbuilder") {
                    $moduleObj = Module::getInstanceByName($module);
                    $moduleObj->importFormsSample($content->sample);
                } else {
                ...
                if ($module == "leoblog") {
//                    $leoblog_image = _PS_ALL_THEMES_DIR_.leoECHelper::getThemeName().'/assets/img/modules/leoblog/';
                    ...
                    if (is_dir($leoblog_sample_image)) {
                        if (!is_dir($leoblog_shop_image)) {
                            ...
                            leoECHelper::fullCopy($leoblog_sample_image, $leoblog_shop_image);
                        }
                    }
                }
            } else {
    ...
    public function importData($content, $objectMName, $searchField, $searchValue, $moduleName, $parentID = 0)
    {
        ...
        if ($objectMName == 'ApPageBuilderProfilesModel') {
            ...
            $correct_mobile = Db::getInstance()->executeS('SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = "'._DB_NAME_.'" AND TABLE_NAME="'._DB_PREFIX_.'leoelements_profiles_shop" AND column_name="active_mobile"');
            if (count($correct_mobile) < 1) {
                Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'leoelements_profiles_shop` ADD `active_mobile` int(11) NOT NULL');
                Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'leoelements_profiles_shop` ADD `active_tablet` int(11) NOT NULL');
            }

            //nghiatd - add theme mobile and table
            $correct_mobile = Db::getInstance()->executeS('SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = "'._DB_NAME_.'" AND TABLE_NAME="'._DB_PREFIX_.'leoelements_profiles" AND column_name="active_mobile"');
            if (count($correct_mobile) < 1) {
                Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'leoelements_profiles` ADD `active_mobile` int(11) NOT NULL');
                Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'leoelements_profiles` ADD `active_tablet` int(11) NOT NULL');
            }


            //nghiatd - add theme mobile and table
            $correct_mobile = Db::getInstance()->executeS('SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = "'._DB_NAME_.'" AND TABLE_NAME="'._DB_PREFIX_.'leoelements_details_shop" AND column_name="active_mobile"');
            if (count($correct_mobile) < 1) {
                Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'leoelements_details_shop` ADD `active_mobile` int(11) NOT NULL');
                Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'leoelements_details_shop` ADD `active_tablet` int(11) NOT NULL');
            }

            //nghiatd - add theme mobile and table
            $correct_mobile = Db::getInstance()->executeS('SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = "'._DB_NAME_.'" AND TABLE_NAME="'._DB_PREFIX_.'leoelements_details" AND column_name="active_mobile"');
            if (count($correct_mobile) < 1) {
                Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'leoelements_details` ADD `active_mobile` int(11) NOT NULL');
                Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'leoelements_details` ADD `active_tablet` int(11) NOT NULL');
            }
            
            //nghiatd - add theme mobile and table
            $correct_mobile = Db::getInstance()->executeS('SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = "'._DB_NAME_.'" AND TABLE_NAME="'._DB_PREFIX_.'leoelements_products_shop" AND column_name="active_mobile"');
            if (count($correct_mobile) < 1) {
                Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'leoelements_products_shop` ADD `active_mobile` int(11) NOT NULL');
                Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'leoelements_products_shop` ADD `active_tablet` int(11) NOT NULL');
            }

            //nghiatd - add theme mobile and table
            $correct_mobile = Db::getInstance()->executeS('SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = "'._DB_NAME_.'" AND TABLE_NAME="'._DB_PREFIX_.'leoelements_products" AND column_name="active_mobile"');
            if (count($correct_mobile) < 1) {
                Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'leoelements_products` ADD `active_mobile` int(11) NOT NULL');
                Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'leoelements_products` ADD `active_tablet` int(11) NOT NULL');
            }
...
//                }
        ...
        if ($moduleList) {
            foreach ($data->modules->children() as $moduleXml) {
                ...
                if (in_array((string)$moduleXml->name, $moduleList)) {
                    ...
                    if (isset($moduleXml->fields) && $moduleXml->name != 'leoformbuilder') {
                        foreach ($moduleXml->fields->children() as $field) {
                            if($field->objectMName == 'LeoElementsRevisions') {
                                continue;
                            }
                    ...
                    if ($moduleXml->name == 'leoformbuilder') {
                        $leoformbuilder = Module::getInstanceByName('leoformbuilder');
                        $this->_content .= $leoformbuilder->exportFormsSample();
                    }
                    //export create table
    ...
    public static function getProfileStatus()
    {
        ...
        $sql = 'SELECT * FROM `'._DB_PREFIX_.'leoelements_profiles_shop` WHERE id_shop='.(int)$id_shop;
        $result = Db::getInstance()->executes($sql);
        $profiles = array();
        foreach ($result as $key => $val) {
            $profiles[$val['id_leoelements_profiles']] = $val['active'];
        }
    ...
    {
        $id_shop = (int)Context::getContext()->shop->id;
        $sql = 'SELECT * FROM `'._DB_PREFIX_.'leoelements_products_shop` WHERE id_shop='.(int)$id_shop;
        $result = Db::getInstance()->executes($sql);
        $profiles = array();
        foreach ($result as $key => $val) {
            $profiles[$val['id_leoelements_products']] = $val['active'];
        }
        ...
        foreach ($data->modules as $modules) {
            foreach ($modules as $module) {
                ...
                if ((Module::isInstalled((string)$module->name) || $onlyModName) && file_exists(_PS_MODULE_DIR_.(string)$module->name)) {
                    ...
                    if (Module::isInstalled((string)$module->name)) {
                        $url = 'index.php?controller=adminmodules&configure='.$module->name.'&token='.Tools::getAdminTokenLite('AdminModules')
                .'&tab_module=Home&module_name='.$module->name;

                        $moduleList[(string)$module->name]["url"] = $url;
                    }
                    
                    if (file_exists(_PS_ALL_THEMES_DIR_.leoECHelper::getThemeName().'/samples/'.$module->name.'.xml')) {
                        $link = Context::getContext()->shop->physical_uri.'themes/'.leoECHelper::getThemeName().'/samples/';
                        $moduleList[(string)$module->name]["sample"] = $link.$module->name.'.xml';
                    }
                }
            }
        }
    ...
    public function exportDBStruct()
    {
        ...
        //copy + export to
        $installFolder = _PS_MODULE_DIR_."leoelements/install";
        ...
        if ($fp === false) {
            $this->_html["error"] = "Error when export DbStruct! Can not write file in leoelements/install";
            return false;
        }
...
//            _DB_PREFIX_.'contact', _DB_PREFIX_.'contact_shop'
            ...
            if (!in_array($schema[0]['Table'], $ignore_insert_table)) {
                ...
                if ($data && $sizeof > 0) {
                    ...
                    while ($row = DB::getInstance()->nextRow($data)) {
                        ...
                        if (($schema[0]['Table'] == _DB_PREFIX_."leoelements_lang" && $i % 10 == 0) && $i < $sizeof) {
                            # Insert 1 time have 10 records, only leoelements_lang table
    ...
    function processBackUp()
    {
        ...
        $moduleList = Tools::getValue("bumodule");
        if ($moduleList) {
            foreach ($data->modules->children() as $moduleXml) {
                //only create sample for module if select
                if (in_array((string)$moduleXml->name, $moduleList)) {
                    ...
                    # EXPORT CONFIG
                    if ($moduleXml->config_prefix) {
                        if (!$this->_currentBackupFile) {
                            $this->_currentBackupFile = $installFolder.(string)$moduleXml->name.date('_Y_m_d_H_i_s').".php";
                        }
    ...
    public function createConfigSample($module, $configPrefix)
    {
        ...
        $configData = $start_file."\n\$dataConfig = Array(";
        foreach ($data as $key => $val) {
            if ($configData != $start_file."\n\$dataConfig = Array(") {
                $configData .= ",\"";
            } else {
                $configData .= "\"";
            }
        ...
        while ($row = DB::getInstance()->nextRow($data)) {
            ...
            foreach ($row as $field => $value) {
                if ($field == "id_shop") {
                    ...
                    $tmp = "\"".pSQL(str_replace('"', '\'', $value), true)."\",";
                }
                if ($tmp != "'',") {
                    $s .= $tmp;
                } else {
    ...
    public function installSampleModule($file, $module = '')
    {
        ...
        if (isset($query) && !empty($query)) {
            ...
            foreach ($db_data_settings as $query) {
                $query = trim($query);
                if (!empty($query)) {
                    if (strpos($query, 'leoelements_meta') == false) {
                        if (strpos($query, 'leoelements_products') != false
                            || strpos($query, 'leoelements_product_list') != false
                            || strpos($query, 'leoelements_positions') != false) {
                            $query = str_replace("'", '\"', $query);
                        } else {
                            $query = str_replace("\'", '\\\\\"', $query);
                        }
            ...
            // convert params column to correct data
            $profiles = DB::getInstance()->executeS('SELECT * FROM '._DB_PREFIX_.'leoelements_profiles WHERE 1');
            foreach ($profiles as $profile) {
                DB::getInstance()->execute('UPDATE '._DB_PREFIX_.'leoelements_profiles SET params=\''.str_replace('\"', '\\\\\"', str_replace("'", '"', $profile['params'])).'\' WHERE id_leoelements_profiles='.$profile['id_leoelements_profiles']);
            }
        }
        ...
        if (isset($dataLang) && !empty($dataLang)) {
            ...
            foreach ($languages as $lang) {
                ...
                foreach ($db_data_settings as $query) {
                    $query = trim($query);
                    if (!empty($query)) {
                        if (strpos($query, 'leoelements_contents_lang') != false) {
                            $query = str_replace("\\r\\n",'\\\r\\\n', str_replace('\"', '\\\\\"', $query));
                        }
    ...
    {
        if (leoECHelper::leoExitsDb('table', 'btmegamenu_widgets')) {
