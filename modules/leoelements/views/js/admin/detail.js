...
$(document).ready(function() {
    ...
    $(".adminleoelementdetails #nav-sidebar").trigger("click");
    ...
    $('.plist-eedit').click(function(){
        ...
        $.fancybox.open([{
                type: 'iframe', 
                href : ($('#leoelements_products_form').length?$('#leoelements_products_form').attr('action'):$('#leoelements_products_form').attr('action')) + '&pelement=' + element,
                afterLoad:function(){
    ...
    $('#leoelements_products_form').submit(function() {
        genreateForm();
    });

    //DONGND:: setup ui for element
    $('.leoelement_Grid .row, .leoelement_Grid .column-row').addClass('ui-widget ui-widget-content ui-helper-clearfix ui-corner-all');
    $('.leoelement_Grid .gaction-drag, .leoelement_Grid .caction-drag, .leoelement_Grid .waction-drag').addClass('ui-widget-header ui-corner-all');	
});
...
function replaceSpecialString(str){
    return str.replace(/\t/g, "_APTAB_").replace(/\r/g, "_APNEWLINE_").replace(/\n/g, "_APENTER_").replace(/"/g, "_APQUOT_").replace(/'/g, "_APAPOST_").replace(/\$/g, "_APDOLA_");
}
function saveData(){
