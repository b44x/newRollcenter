...
<div id="leo-page" class="clearfix">
	...
	<div class="note">
		...
		<p>+ {l s='Drop modules from hooks layouts to "<b>UnHook Modules</b>" Panel to unhook them'  mod='leoelements'}. {l s='Drag and drop modules from hooks layouts to update theirs order and hook position'  mod='leoelements'}</p>
		<p>+  {l s='Override hook feature only applies for <b>HOOK_HEADERRIGHT, HOOK_SLIDESHOW, HOOK_TOPNAVIATION, HOOK_SLIDESHOW, HOOK_PROMOTETOP, HOOK_CONTENTBOTTOM, HOOK_BOTTOM</b>'  mod='leoelements'}</p>
		<p>+ {l s='Here only shows all of installed modules having hooks supportting for LeoTheme Layout.' mod='leoelements'}
	</div>	
	<div class="leo-container holdposition" id="noposition">
		<div class="pos">{l s='UnHook Modules'  mod='leoelements'} </div>
...
<div id="overidehook" style="display:none">
	...
	<p class="clearfix"><label>{l s='Select override hook' mod='leoelements'}</lable><br>
	<select  name="name_hook">
		<option value="0">{l s='--- Use Self Hook ---'  mod='leoelements'}</option>
	...
	<input type="submit" value="{l s='Save' mod='leoelements'}" name="submit" />
	</p>
	</form>
</div>	
