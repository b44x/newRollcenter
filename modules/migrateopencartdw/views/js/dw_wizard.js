...
(function ($) {
        ...
        $('.buttonResume').on('click', function () {
            ...
            $.ajax({
                ...
                success: function (datas) {
                    $("#dw_wizard").smartWizard('disableStep', 1);
                    $("#dw_wizard").smartWizard('disableStep', 2);
                    $("#dw_wizard").smartWizard('enableStep', 3);
                    $("#step-1").hide();
                    $("#step-2").hide();
                    $("#step-3").show();
                    $('.actionBar .buttonNext').hide();
                    $("#dw_wizard").smartWizard('setCurrentStep', 3);
    ...
    function onFinishCallback(obj, context) {
        ...
        $.ajax({
            ...
            async: false,
            dataType: 'json',
            data: $('#dw_wizard .stepContainer .content form').serialize() + '&to_step_number=' + context.fromStep + '&action=import_process&ajax=1&step_number=' + context.fromStep,
            success: function (datas) {
    ...
    function validateSteps(fromStep, toStep) {
        ...
        $('.wizard_error').remove();
        if (is_ok) {
            var form = $('#dw_wizard #step-' + fromStep + ' form');
            $.ajax({
                type: "POST",
                url: validate_url,
                async: false,
    ...
    function displayWarning(warnings, step_number) {
        $('#dw_wizard .actionBar a.btn').removeClass('disabled');
        $('.warning.wizard_error').remove();
        var str_warning = '<div class="warning wizard_error alert alert-danger" style="display:none"><ul>';
        for (var warning in warnings) {
            str_warning += '<li>' + warnings[warning] + '</li>';
        }
        str_warning += '<p><a class="dw-warning-log" style="cursor:pointer" target="_blank" href=' + warningLogPath + '>Download log file</a></p>';
    ...
    function displayError(errors, step_number) {
        $('#dw_wizard .actionBar a.btn').removeClass('disabled');
        $('.error.wizard_error').remove();
        var str_error = '<div class="error wizard_error alert alert-danger" style="display:none"><ul>';
    ...
    function displayForm(step_form, step_number) {
        var form = $('#dw_wizard #step-' + step_number);
        form.html(step_form);
        if (step_number == 2) {
    ...
    function resizeWizard() {
        var resizeInterval = setInterval(function () {
            $("#dw_wizard").smartWizard('fixHeight');
    ...
    function bind_inputs() {
        ...
        $('input, select').focus(function () {
            $('#dw_wizard .actionBar a.btn').not('.buttonFinish').removeClass('disabled');
        });
    ...
    function changePercentByEntityType(currentProcess) {
        var processParentElement = $("#" + currentProcess.type);
