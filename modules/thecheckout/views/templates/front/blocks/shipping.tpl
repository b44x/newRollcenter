              ...
              {block name='delivery_options'}
                <div class="delivery-options">
                  {foreach from=$delivery_options item=carrier key=carrier_id}
                    ...
                    <div
                            class="row js-carrier-extra-content carrier-extra-content{if isset($carrier.external_module_name) && "" != $carrier.external_module_name} {$carrier.external_module_name|escape:'javascript':'UTF-8'}{/if}"{if $delivery_option != $carrier_id} style="display:none;"{/if}>{$carrier.extraContent nofilter}</div>
