	...
	<div class="leo-sale">
		{if $product.has_discount}
		    ...
		    {elseif $product.discount_type === 'amount'}
				<label class="label product-flag discount-amount discount-product">{$product.discount_amount_to_display}</label>
			{/if}
	    {/if}
	</div>
    
      {if $product.quantity == 0}
        <span class="product-out-stock">{l s='Out of stock' d='Shop.Theme.Global'}</span>
      {else}
        <span class="product-in-stock">{l s='In stock' d='Shop.Theme.Global'}</span>
      {/if}
