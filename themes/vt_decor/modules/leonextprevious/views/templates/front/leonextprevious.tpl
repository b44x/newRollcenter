...
{if $position != 3 || (isset($buttons) && $buttons)}<div class="leonextprevious-container clearfix">{/if}
	{if $prev}
		<div class="leo-button-prev">
			...
			<div>{if $display_image}<img src="{Context::getContext()->link->getImageLink($prev.link_rewrite, $prev.id_image, 'small_default')}" title="{$prev.legend}">{/if}
				{if $display_names || $display_price}<p class="small"><span>{if $display_names}{$prev.name}<br>{/if}</span>{if $display_price}{$prev.price_static}<br>{/if}</p class="small">{/if}</div>
		</div>
	...
	{if $next}
		<div class="leo-button-next">
			...
			<div>{if $display_image}<img src="{Context::getContext()->link->getImageLink($next.link_rewrite, $next.id_image, 'small_default')}" title="{$next.legend}">{/if}
				{if $display_names || $display_price}<p class="small"><span>{if $display_names}{$next.name}<br>{/if}</span>{if $display_price}{$next.price_static}<br>{/if}</p class="small">{/if}</div>
		</div>
