        ...
        <div class="row">
          <div class="col-md-12">
            ...
            {if isset($order_customer.email)}
              {assign var="customer_email" value=$order_customer.email}
            {else}
              {assign var="customer_email" value=$customer.email}
            {/if}
            <p>
              {l s='An email has been sent to your mail address %email%.' d='Shop.Theme.Checkout' sprintf=['%email%' => $customer_email]}
...
{block name='page_content_container'}
    ...
    <div class="card-block">
      <div class="row">
		...
		<li id="order-reference-value">
  			{l s='Numer zamówienia: %number%' d='Shop.Theme.Checkout'
    			sprintf=[
      			'%number%' => $order.details.reference
    				]
  			}
		</li>
  ...
  {if isset($registered_customer_exists) && !$registered_customer_exists}
    ...
    {/block}
  {elseif isset($customer.is_guest) && $customer.is_guest}
